...
export function Advantages({ lang }: { lang: string }) {
  ...
  return (
    <section
      id="advantages"
      className="py-24 scroll-mt-20 bg-linear-to-br from-slate-900 via-blue-900 to-slate-900 relative overflow-hidden"
    >
      ...
      <div className="container mx-auto px-4 relative z-10">
        <div className="text-center mb-16">
          ...
          <h2 className="text-5xl md:text-7xl font-extrabold text-white mb-6 drop-shadow-2xl">
            {t('advantages.title')}{" "}
            <span className="text-transparent bg-clip-text bg-linear-to-r from-amber-300 via-fuchsia-500 to-rose-500 bg-size-[200%_200%] drop-shadow-[0_0_18px_rgba(251,191,36,0.55)]">
          ...
          <div className="flex flex-wrap gap-8 justify-center items-stretch">
            {ADVANTAGES.map((c, i) => (
              ...
              >
                ...
                >
                  <div
                    className={`absolute inset-0 bg-linear-to-br ${c.color} opacity-5 group-hover:opacity-15 transition-all duration-500`}
                  />
                  <div
                    className={`absolute top-0 right-0 w-48 h-48 bg-linear-to-br ${c.color} opacity-10 rounded-full blur-3xl -mr-24 -mt-24 group-hover:opacity-30 transition-all group-hover:scale-150 duration-700`}
                  />
                  ...
                  <CardContent className="px-10 py-4 relative flex-1 flex flex-col">
                    <div className="flex items-center gap-4 mb-6">
                      <div
                        className={`bg-linear-to-br ${c.color} p-4 rounded-2xl shadow-2xl group-hover:shadow-amber-500/50 transition-all group-hover:scale-110 group-hover:rotate-6 duration-500`}
                      >
                        <c.icon className="w-8 h-8 text-white" />
                      </div>
                      <div>
                        <h3 className="text-2xl font-black text-slate-900 group-hover:text-transparent group-hover:bg-clip-text group-hover:bg-linear-to-r group-hover:from-amber-600 group-hover:to-orange-600 transition-all duration-300">
                          {c.title}
                        </h3>
                    ...
                    {c.tags.length > 0 && (
                      <div className="flex flex-wrap gap-2 mt-4">
                        {c.tags.map((tagText) => (
                          <span
                            key={tagText}
                            className="bg-linear-to-r from-amber-100 to-orange-100 text-amber-800 px-4 py-2 rounded-full text-sm font-bold border-2 border-amber-300 shadow-sm hover:shadow-md hover:scale-105 transition-all duration-300"
                          >
                            {tagText}
                          </span>
                        ))}
